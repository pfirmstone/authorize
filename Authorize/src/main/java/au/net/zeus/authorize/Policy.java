...
public abstract class Policy {
    ...
    private static final Guard SET_POLICY = new SecurityPermission(
            "setPolicy"); //$NON-NLS-1$

    // The SecurityPermission required to get current Policy.
    static final Guard GET_POLICY = new SecurityPermission(
            "getPolicy"); //$NON-NLS-1$

    // The policy currently in effect. 
    private static volatile Policy activePolicy;
    ...
    private static Policy getDefaultProvider() {
        final String defaultClass = AccessController
                .doPrivileged(new SecurityPropertyAccessor(
                        POLICY_PROVIDER));
    ...
    private static class DefaultPolicy extends Policy {
